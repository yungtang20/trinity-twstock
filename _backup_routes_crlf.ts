...
import { mvpMcpHandler, jobBatchHandler, jobGetHandler, jobListHandler, tdccSyncHandler, tdccStatusHandler } from "./mvpMcpRoutes";
import {
  pushTdccToSupabase,
  pushPriceToSupabase,
...
function fetchEngineRows(stockId: string): any[] {
  ...
  if (!db) return [];
  return db.prepare(
    "SELECT date, open, high, low, close, volume FROM stock_history WHERE stock_id = ? ORDER BY date DESC LIMIT 1000"
  ).all(stockId).reverse();
}
...
router.get("/api/stock/:id/history", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const countRow = db.prepare("SELECT count(*) as c FROM stock_history WHERE stock_id = ?").get(id) as any;
...
router.get("/api/stock/:id/institutional", async (req: Request, res: Response) => {
  ...
  try {
    let rows = db.prepare(
      "SELECT date, foreign_net, trust_net, dealer_net, institutional_net FROM institutional_data WHERE stock_id = ? ORDER BY date DESC LIMIT 1000"
    ).all(id) as any[];
...
router.get("/api/stock/:id/quote", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const latest = db.prepare("SELECT * FROM stock_history WHERE stock_id = ? ORDER BY date DESC LIMIT 1").get(id) as any;
    if (!latest) return res.json({ success: false, error: "No price data" });

    const prev = db.prepare("SELECT * FROM stock_history WHERE stock_id = ? ORDER BY date DESC LIMIT 1 OFFSET 1").get(id) as any;
    const hist = db.prepare("SELECT date, open, high, low, close, volume FROM stock_history WHERE stock_id = ? ORDER BY date DESC LIMIT 1000").all(id).reverse() as any[];
    const indicators = calcIndicators(hist);
...
router.get("/api/sync-status", (_req: Request, res: Response) => {
  ...
  if (db) {
    try {
      const latestDbRow = db.prepare("SELECT MAX(date) as max_date FROM stock_history").get() as { max_date: string | null };
...
// REMOVED: /api/test-key — 洩露 API Key，已移除
router.get("/api/settings", async (_req: Request, res: Response) => {
  const sb = await loadSettingsFromSupabase();
  res.json({
    success: true,
    longcatApiKey: (sb.longcatApiKey || process.env.VITE_LONGCAT_API_KEY) ? "已配置" : "",
    ...
    finmindApiKey: (sb.finmindApiKey || process.env.VITE_FINMIND_API_KEY) ? "已配置" : "",
    ...
    if (supabase && insertedRecords > 0) {
      try {
        ...
        for (let s = 0; s < supabaseRows.length; s += bSize) {
          ...
          const { error: sbErr } = await supabase.from("tdcc_shareholding").upsert(batch);
          if (sbErr) {
            throw sbErr;
          }
        }
...
router.get("/api/movers", (_req: Request, res: Response) => {
  ...
  if (!db) return res.json({ success: false, error: "DB not connected" });
  try {
    const latestDateRow = db.prepare(`SELECT date FROM stock_history GROUP BY date HAVING COUNT(*) > 100 ORDER BY date DESC LIMIT 1`).get() as any;
    const latestDate = latestDateRow?.date || (db.prepare("SELECT MAX(date) as d FROM stock_history").get() as any)?.d;
    if (!latestDate) return res.json({ success: false, error: "No data" });

    const prevDateRow = db.prepare(`SELECT date FROM stock_history WHERE date < ? GROUP BY date HAVING COUNT(*) > 100 ORDER BY date DESC LIMIT 1`).get(latestDate) as any;
    const prevDate = prevDateRow?.date || (db.prepare("SELECT MAX(date) as d FROM stock_history WHERE date < ?").get(latestDate) as any)?.d;
...
router.get("/api/dashboard/recent-dividend", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const latestDateRow = db.prepare(`SELECT MAX(date) as d FROM stock_history`).get() as { d: string } | undefined;
    ...
    const formatted = events.map((ev: any, idx: number) => {
      const hist = db.prepare(`
        ...
        LEFT JOIN stock_history h_prev ON h.stock_id = h_prev.stock_id 
          AND h_prev.date = (SELECT MAX(date) FROM stock_history WHERE date < ?)
...
router.get("/api/dashboard/trust-buy-2day", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const trustBuyStocks = db.prepare(`
      ...
      JOIN stock_history h ON i.stock_id = h.stock_id AND h.date = i.date
    ...
    const formatted = trustBuyStocks.map((s: any) => {
      ...
      for (let j = 2; j < datesRow.length; j++) {
        const row = db.prepare(`
          SELECT trust_net FROM institutional_data WHERE stock_id = ? AND date = ?
...
router.get("/api/dashboard/break-ma200", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const getHistoryStmt = db.prepare(`
      SELECT close FROM stock_history WHERE stock_id = ? ORDER BY date DESC LIMIT 202
    `);
  ...
  try {
    ...
    const formatted = limitUpStocks.map((s: any) => {
      ...
      const avgVolRow = db.prepare(`
        SELECT AVG(volume) as avg_vol FROM stock_history WHERE stock_id = ? AND date <= ? ORDER BY date DESC LIMIT 5
...
router.get("/api/stock/:id/sr-analysis", async (req: Request, res: Response) => {
  ...
  const id = req.params.id;
  try {
    const latest = db.prepare("SELECT date, close FROM stock_history WHERE stock_id = ? ORDER BY date DESC LIMIT 1").get(id) as any;
    if (!latest) return res.json({ success: false, error: "No price data" });
...
router.get("/api/stock/:id/ma-analysis", async (req: Request, res: Response) => {
  ...
  try {
    const rows = db.prepare(
      "SELECT date, close FROM stock_history WHERE stock_id = ? ORDER BY date DESC LIMIT 1000"
    ).all(id).reverse() as any[];
...
router.get("/api/stock/:id/chips-analysis", async (req: Request, res: Response) => {
  ...
  const id = req.params.id;
  try {
    const latestRow = db.prepare("SELECT MAX(date) as d FROM stock_history WHERE stock_id = ?").get(id) as any;
...
router.get("/api/stock/:id/prediction-analysis", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      ...
      isSimulated: true,
      disclaimer: "⚠️ 此分析基於統計模擬，非真實 AI/ML 模型預測，僅供參考，不構成投資建議。"
    });
  } catch (err: any) {
    res.json({ success: false, error: err.message });
  }
...
router.get("/api/strategy/sr-scan", async (req: Request, res: Response) => {
  ...
  try {
    const latestDate = (db.prepare("SELECT MAX(date) as d FROM stock_history").get() as any)?.d;
...
router.get("/api/strategy/ma-scan", async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const c of candidates) {
      await new Promise(r => setImmediate(r)); // Yield event loop to avoid blocking
      const rows = db.prepare(
        "SELECT close FROM stock_history WHERE stock_id = ? ORDER BY date DESC LIMIT 1000"
...
router.get("/api/strategy/chips-scan", async (req: Request, res: Response) => {
  ...
  try {
    const latestDate = (db.prepare("SELECT MAX(date) as d FROM stock_history").get() as any)?.d;
    const instDate = (db.prepare("SELECT MAX(date) as d FROM institutional_data").get() as any)?.d;
...
router.get("/api/strategy/pattern-scan", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({ success: true, data: results.slice(0, limit), isSimulated: true, disclaimer: "⚠️ 型態辨識基於統計模擬，非真實圖形辨識模型，僅供參考。" });
  } catch (err: any) {
    res.json({ success: false, error: err.message });
  }
});
...
// 新 job-based 路由 (分頁切換仍继xu作анализ)
