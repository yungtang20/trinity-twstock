...
if (!url || !key) {
  console.error("❌ Need VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to run!");
  process.exit(1);
}
...
// ✅ 動態 280 天滾動視窗（與 taipeiNow 同一個 timezone 基準）
const cutoffTaipei = new Date(
  new Date(Date.now() - 280 * 24 * 60 * 60 * 1000)
    .toLocaleString("en-US", { timeZone: "Asia/Taipei" })
);
const cy  = cutoffTaipei.getFullYear();
const cm  = String(cutoffTaipei.getMonth() + 1).padStart(2, '0');
const cdd = String(cutoffTaipei.getDate()).padStart(2, '0');
const CUTOFF_DATE = `${cy}-${cm}-${cdd}`;
...
async function run() {
  ...
  while (hasMore) {
    ...
    if (e4) {
      console.error("Failed to query stock_features from Supabase:", e4.message);
      break;
    }
  ...
  // 2.1 TWSE Quotes
  try {
    const twseUrl = `https://www.twse.com.tw/exchangeReport/MI_INDEX?response=json&date=${TODAY_DATE_NODASH}&type=ALL`;
  ...
  if (todayPrices.length > 0) {
    ...
    console.log("✅ SQLite stock_history updated with today's prices!");
  }
  ...
  if (instList.length > 0) {
    ...
    console.log("✅ SQLite institutional_data updated with today's volume!");
  }
  ...
  console.log(`✅ Loaded ${dates.length} unique trading days into SQLite calendar, ending on ${dates[dates.length - 1]?.date}!`);
