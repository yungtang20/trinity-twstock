# TRINITY 台股分析平台

**報告日期**：2026-06-21
**版本**：v2.0
**狀態**：開發中

---

## 文件導覽

本專案的開發規範文件位於 `twstock/` 目錄。

| 文件 | 用途 |
|------|------|
| [AGENTS.md](../twstock/AGENTS.md) | AI Agent 啟動入口 |
| [DOCUMENT_INDEX.md](../twstock/DOCUMENT_INDEX.md) | 一頁導航所有文件 |
| [ARCHITECTURE.md](../twstock/ARCHITECTURE.md) | 完整架構與規範 |
| [PROJECT_RULES.md](../twstock/PROJECT_RULES.md) | 專案開發規範 |
| [DEVELOPMENT_GUIDE.md](../twstock/DEVELOPMENT_GUIDE.md) | 新增功能流程 |
| [AI_CHECKLIST.md](../twstock/AI_CHECKLIST.md) | 修改完成驗收 |
| [DB_SCHEMA.md](../twstock/DB_SCHEMA.md) | 資料庫 Schema |
| [API_SPEC.md](../twstock/API_SPEC.md) | 外部 API 規格 |
| [SYSTEM_OVERVIEW.md](../twstock/SYSTEM_OVERVIEW.md) | 系統架構圖 |
| [JSON_CONTRACT.md](../twstock/JSON_CONTRACT.md) | JSON 輸出合約 |
| [PERFORMANCE_RULES.md](../twstock/PERFORMANCE_RULES.md) | 效能規範 |
| [TEST_DATA.md](../twstock/TEST_DATA.md) | 測試資料規範 |
| [DEPENDENCIES.md](../twstock/DEPENDENCIES.md) | 相依套件版本 |
| [CHANGELOG.md](../twstock/CHANGELOG.md) | 變更歷史 |
| [VERSION.md](../twstock/VERSION.md) | 版本資訊 |

---

## 如何更新資料

```bash
# 進入 twstock 目錄
cd ../twstock

# 每日更新（最新 5 個交易日）
python main.py official --days 5 --with-tdcc

# 單股策略分析
python main.py strategy --strategy-id 1 --code 2330

# 全市場掃描
python main.py strategy --strategy-id 1 --scan --vol 500
```

---

## 如何新增策略

1. 複製 `strategy/templates/strategy_template.py`
2. 實作 `analyze()` / `run_strategy()` / `scan_market()`
3. 在 `strategy/strategies.py` 的 `STRATEGY_REGISTRY` 註冊
4. 執行 `python strategy_runner.py <stock_id>` 測試
5. 詳細流程請見 [DEVELOPMENT_GUIDE.md](../twstock/DEVELOPMENT_GUIDE.md)

---

## 文件索引

完整文件索引請見 [DOCUMENT_INDEX.md](../twstock/DOCUMENT_INDEX.md)。

---

## 1. 平台概述

TRINITY 台股分析平台是一個全方位的股票分析工具，提供即時行情、技術分析、籌碼分析、策略掃描和 AI 智慧分析功能。

### 核心功能
- **即時行情**：即時股票價格、漲跌幅、成交量
- **技術分析**：K 線圖、均線（MA5/25/60/200）、支撐壓力、POC、VSBC
- **籌碼分析**：法人買賣超、千張大戶、集保人數
- **策略掃描**：支撐壓力、均線、籌碼、型態、預測
- **AI 智慧分析**：9 種華爾街分析模板

### 目標用戶
- 台股投資者
- 技術分析愛好者
- 量化交易者

---

## 2. 頁面架構

### 2.1 儀表板（Dashboard）

```
┌─────────────────────────────────────────────────────────────────┐
│  標題列：市場總覽 | 即時更新                                       │
├─────────────────────────────────────────────────────────────────┤
│  ┌─────────────┐  ┌─────────────┐                                │
│  │ 加權指數     │  │ 櫃買指數     │                                │
│  │ 即時/收盤    │  │ 即時/收盤    │                                │
│  │ 漲跌幅      │  │ 漲跌幅      │                                │
│  │ 成交金額    │  │ 成交金額    │                                │
│  │ 漲/平/跌    │  │ 漲/平/跌    │                                │
│  └─────────────┘  └─────────────┘                                │
├─────────────────────────────────────────────────────────────────┤
│  ┌─────────────┐  ┌─────────────────────────────┐                │
│  │ 市場狀態     │  │ 漲幅/跌幅排行                │                │
│  │ 台北時間     │  │ (MoversList)                │                │
│  │ 最後更新     │  │                             │                │
│  └─────────────┘  └─────────────────────────────┘                │
└─────────────────────────────────────────────────────────────────┘
```

**數據來源**：
- 加權指數/櫃買指數：Supabase + FinMind 即時數據
- 漲跌幅排行：Supabase stock_history

---

### 2.2 市場分析（Markets）

```
┌─────────────────────────────────────────────────────────────────┐
│  [搜尋框: 輸入股號或股名]  [搜尋]  [AI 分析]  DB: 2026-06-18     │
├─────────────────────────────────────────────────────────────────┤
│  台積電 (2330)  FinMind  | 日期: 2026-06-18 | 即時數據           │
│  NT$ 2,410.00  +1.05% (+25.00)                                 │
├─────────────────────────────────────────────────────────────────┤
│ K線: [日] [週] [月]    MA: [5] [25] [60] [200]                   │
│ 支撐壓力: [前高/前低] [短期] [長期] [POC] [VSBC] [關閉]         │
├─────────────────────────────────────────────────────────────────┤
│                        K 線圖 (包含成交量)                       │
├─────────────────────────────────────────────────────────────────┤
│ 成交量: 今日 | 5日均量 | 20日均量                                │
├─────────────────────────────────────────────────────────────────┤
│ [法人投信/外資] [千張大戶/集保人數]                               │
│                    柱狀圖 (ChipChart)                            │
└─────────────────────────────────────────────────────────────────┘
```

**數據來源**：
- 個股報價：FinMind API → Supabase fallback
- K 線圖：FinMind API（2015-01-01 至今日）
- 成交量：FinMind API
- 法人/外資：Supabase institutional_data
- 千張大戶/集保人數：Supabase shareholding_unified

---

### 2.3 策略模組（Strategies）

```
┌─────────────────────────────────────────────────────────────────┐
│  [搜尋框]  [搜尋]  [AI 分析]                                     │
├─────────────────────────────────────────────────────────────────┤
│ 策略類型: [支撐壓力] [均線] [籌碼] [型態] [預測]                 │
├─────────────────────────────────────────────────────────────────┤
│                        策略掃描結果                              │
│  股號 | 股名 | 收盤 | 成交量 | 金額 | 評分                      │
└─────────────────────────────────────────────────────────────────┘
```

---

### 2.4 AI 分析（AI Analysis）

```
┌─────────────────────────────────────────────────────────────────┐
│  AI 智能分析  FinMind                                             │
├──────────────────────┬──────────────────────────────────────────┤
│  數據收集策略        │  股票代號 / 股名                          │
│  Supabase: TDCC      │  [2330        ]                          │
│  FinMind: 7個        │                                          │
│  LongCat: 128K       │  Prompt 8 自訂問題                        │
│  2015-01-01 至今      │  [輸入您的問題...]                        │
├──────────────────────┼──────────────────────────────────────────┤
│  分析模板            │  分析結果                                 │
│  高盛基本面分析       │  ─────────────────────────────            │
│  摩根士丹利技術       │  [分析報告內容...]                        │
│  橋水風險評估       │                                          │
│  摩根大通財報       │                                          │
│  貝萊德股息         │                                          │
│  文藝復興量化         │                                          │
│  D.E. Shaw 期權      │                                          │
│  頂尖產業分析師       │                                          │
│  華爾街分析師         │                                          │
│  [一鍵執行全部]      │                                          │
├──────────────────────┴──────────────────────────────────────────┤
│  執行過程                                                         │
│  [11:30:00] 開始 高盛基本面分析篩選器...                         │
│  [11:30:01] 從 FinMind 調取數據...                               │
│  [11:30:02] TaiwanStockPrice: 1,095 筆                            │
└─────────────────────────────────────────────────────────────────┘
```

---

### 2.5 設定（Settings）

```
┌─────────────────────────────────────────────────────────────────┐
│  設定                                                             │
├─────────────────────────────────────────────────────────────────┤
│  數據更新                                                          │
│  最後更新: 2026-06-18 11:00:00  [手動更新]                        │
├─────────────────────────────────────────────────────────────────┤
│  API 狀態                                                         │
│  FinMind: 已連線  |  Supabase: 已連線  |  LongCat: 已連線         │
├─────────────────────────────────────────────────────────────────┤
│  數據源狀態                                                        │
│  Supabase: 2,041 檔股票                                          │
│  SQLite: 2,041 檔股票, 1,016,849 筆歷史數據                      │
└─────────────────────────────────────────────────────────────────┘
```

---

## 3. 數據來源

### 3.1 數據來源優先級

```
優先級 1：FinMind API（免費會員）
  ↓ 如果失敗或無數據
優先級 2：Supabase
  ↓ 如果失敗或無數據
優先級 3：SQLite 本地資料庫
```

### 3.2 FinMind API 數據集（12 個免費）

| 數據集 | 說明 |
|--------|------|
| TaiwanStockPrice | 股價資訊 |
| TaiwanStockPER | 本益比/殖利率 |
| TaiwanStockDayTrading | 當日沖銷 |
| TaiwanStockInstitutionalInvestorsBuySell | 三大法人買賣超 |
| TaiwanStockMarginPurchaseShortSale | 融資融券 |
| TaiwanStockShareholding | 持股分級 |
| TaiwanStockFinancialStatements | 財務報表 |
| TaiwanStockMonthRevenue | 月營收 |
| TaiwanStockDividend | 股利資訊 |
| USStockPrice | 美股股價 |
| JapanStockPrice | 日股股價 |
| GoldPrice | 黃金價格 |

### 3.3 日期範圍
- **起始日期**：2015-01-01
- **結束日期**：今日（動態計算）
- **最大範圍**：約 11 年

---

## 4. AI 分析數據收集與分析流程

### 4.1 數據收集策略

**數據來源優先級**：
1. FinMind API（免費會員）
2. Supabase
3. SQLite 本地資料庫

**FinMind API 數據集**：12 個免費數據集

**日期範圍**：2015-01-01 至今日（最大化）

### 4.2 單一報告生成流程

```
用戶點擊模板
    ↓
1. 驗證股票代號
    ↓
2. 收集 FinMind 數據
   ├─ 對每個數據集建立 API 請求參數
   ├─ 發送 POST 請求到 /api/finmind-proxy
   └─ 記錄結果（成功/失敗/無數據）
    ↓
3. 檢查數據是否足夠
   ├─ 如果無數據 → 返回「無數據」
   └─ 如果有數據 → 繼續
    ↓
4. LongCat 生成報告
   ├─ 建立提示詞（模板 + 股票代號 + 數據）
   ├─ 發送 POST 請求到 LongCat API
   │   ├─ model: LongCat-2.0-Preview
   │   ├─ max_tokens: 128,000
   │   └─ temperature: 0.7
   └─ 提取報告內容
    ↓
5. 顯示結果
```

### 4.3 LongCat API 請求格式

```json
{
  "model": "LongCat-2.0-Preview",
  "messages": [
    {
      "role": "system",
      "content": "你是一位專業的台股分析師..."
    },
    {
      "role": "user",
      "content": "分析需求：...\n\n股票代號：...\n\n可用數據：..."
    }
  ],
  "temperature": 0.7,
  "max_tokens": 128000
}
```

### 4.4 Prompt 模板對應表

| 模板 | 需要的數據集 |
|------|-------------|
| 高盛基本面 | TaiwanStockPrice, TaiwanStockFinancialStatements, TaiwanStockMonthRevenue, TaiwanStockPER |
| 摩根士丹利技術 | TaiwanStockPrice, TaiwanStockDayTrading, TaiwanStockMarginPurchaseShortSale |
| 橋水風險 | TaiwanStockPrice, TaiwanStockShareholding, TaiwanStockInstitutionalInvestorsBuySell |
| 摩根大通財報 | TaiwanStockPrice, TaiwanStockFinancialStatements, TaiwanStockMonthRevenue, TaiwanStockDividend |
| 貝萊德股息 | TaiwanStockPrice, TaiwanStockDividend, TaiwanStockFinancialStatements |
| 文藝復興量化 | TaiwanStockPrice, TaiwanStockPER, TaiwanStockFinancialStatements, TaiwanStockDayTrading |
| D.E. Shaw 期權 | TaiwanStockPrice, TaiwanStockMarginPurchaseShortSale |
| 頂尖產業 | TaiwanStockPrice, TaiwanStockFinancialStatements, TaiwanStockMonthRevenue |
| 華爾街 | TaiwanStockPrice, TaiwanStockFinancialStatements, TaiwanStockInstitutionalInvestorsBuySell, TaiwanStockShareholding, TaiwanStockPER |

---

## 5. API 使用限制

### 5.1 FinMind API（免費會員）

| 限制 | 說明 |
|------|------|
| 免費數據集 | 12 個 |
| 日期範圍 | 2015-01-01 至今日 |
| 每個數據集 | 只調閱 1 次 |

### 5.2 LongCat API

| 限制 | 說明 |
|------|------|
| max_tokens | 128,000 |
| temperature | 0.7 |
| 模型 | LongCat-2.0-Preview |

### 5.3 Supabase

| 限制 | 說明 |
|------|------|
| 儲存容量 | 500MB |
| 目前使用 | 327MB |

---

## 6. 技術棧

### 6.1 前端
- **框架**：React 18 + TypeScript
- **構建工具**：Vite
- **樣式**：Tailwind CSS
- **圖表**：lightweight-charts
- **動畫**：Framer Motion
- **圖標**：Lucide React

### 6.2 後端
- **運行時**：Node.js + tsx
- **框架**：Express
- **資料庫**：SQLite（better-sqlite3）+ Supabase

### 6.3 外部 API
- **FinMind**：台股數據
- **LongCat**：AI 分析
- **Supabase**：雲端資料庫

---

## 7. 檔案結構

```
D:\twse\twse-app\
├── src\
│   ├── components\
│   │   ├── views\
│   │   │   ├── DashboardView.tsx    # 儀表板
│   │   │   ├── MarketsView.tsx      # 市場分析
│   │   │   ├── StrategiesView.tsx   # 策略模組
│   │   │   ├── AIAnalysisView.tsx   # AI 分析
│   │   │   └── SettingsView.tsx     # 設定
│   │   ├── KlineChart.tsx           # K 線圖組件
│   │   ├── ChipChart.tsx            # 法人/集保柱狀圖
│   │   ├── Sidebar.tsx              # 側邊欄
│   │   ├── Header.tsx               # 標頭
│   │   └── Layout.tsx               # 佈局
│   ├── api\
│   │   └── ai.ts                    # AI API 點
│   ├── lib\
│   │   ├── api.ts                   # API 函數
│   │   ├── indicators.ts            # 技術指標計算
│   │   └── utils.ts                 # 工具函數
│   ├── App.tsx                      # 主應用程式
│   └── main.tsx                     # 入口點
├── server.ts                        # 後端伺服器
└── .env                             # 環境變數
```

---

## 8. 待辦事項

### 8.1 高優先級
- [ ] 確保千張大戶/集保人數數據正確顯示
- [ ] 增加 K 線圖數據日期範圍（2015-01-01 至今日）
- [ ] 確保 AI 分析使用 FinMind 數據

### 8.2 中優先級
- [ ] 優化 API 呼叫次數
- [ ] 加入 API 使用量監控
- [ ] 優化錯誤處理

### 8.3 低優先級
- [ ] 加入更多技術指標
- [ ] 優化圖表效能
- [ ] 加入更多 AI 分析模板

---

**報告生成時間**：2026-06-21
**目前伺服器狀態**：正常運行中
**數據更新日期**：2026-06-18
