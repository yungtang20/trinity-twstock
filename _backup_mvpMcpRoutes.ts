...
import { startJob, getJob, listJobs } from "./lib/jobQueue";
...
export async function runFrameworkAnalysis(stockId: string, frameworkId: string): Promise<string> {
  ...
  const hasGemini = !!rawGeminiKey && rawGeminiKey.startsWith("AIzaSy");
  ...
  // 1. Prioritize Gemini if a valid key is detected (since the user configured their real key)
  if (hasGemini) {
    try {
  ...
  // 2. Try LongCat if Gemini was not available or if Gemini failed
  if (hasLongcat) {
    try {
...
export async function jobBatchHandler(req: Request, res: Response) {
  ...
  if (!hasLongcat && !geminiKey) return res.status(500).json({ success: false, error: "未偵測到 LongCat API 金鑰，請先到 設定 中配置。" });
