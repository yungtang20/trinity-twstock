...
async function syncDailyPrices() {
  ...
  if (dates && dates.length > 0) {
    ...
    // 1. 清理 stock_price 中的過期數據，或者非 4 位數字代號的無效代號
    const { error: err1 } = await supabase.from("stock_price").delete().or(`date.lt.${cutoffDate},stock_id.not.match.^\\d{4}$`);
    if (err1) console.error("[Sync] Price table cleanup error:", err1.message);
    
    // 2. 清理 stock_institutional
    const { error: err2 } = await supabase.from("stock_institutional").delete().or(`date.lt.${cutoffDate},stock_id.not.match.^\\d{4}$`);
    if (err2) console.error("[Sync] Institutional table cleanup error:", err2.message);
    
    // 3. 清理 stock_features
    const { error: err3 } = await supabase.from("stock_features").delete().or(`date.lt.${cutoffDate},stock_id.not.match.^\\d{4}$`);
    if (err3) console.error("[Sync] Features table cleanup error:", err3.message);
    
    console.log("[Sync] Cleanup complete, old and non-standard stock data pruned from all tables.");
  } else {
