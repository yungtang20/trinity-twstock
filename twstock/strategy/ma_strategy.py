...
def _render_mobile_ma(data, code, name):
    ...
    if ma25:
        d25 = ma25['deduction']
        console.print(f"MA25  {ma25['ma']:.2f}  {ma25['trend']}  "
                       f"扣抵 {d25:.2f} {ma25['tomorrow']}") # [AI MOD]

    # MA60 with deduction
    d60 = ma60['deduction']
    console.print(f"MA60  {ma60['ma']:.2f}  {ma60['trend']}  "
                   f"扣抵 {d60:.2f} {ma60['tomorrow']}")

    # MA200 with deduction
    d200 = ma200['deduction']
    console.print(f"MA200 {ma200['ma']:.2f}  {ma200['trend']}  "
                   f"扣抵 {d200:.2f} {ma200['tomorrow']}")
...
def _render_full_ma(data, code, name):
    ...
    if ma25:
        t.add_row("MA25 (月線)", f"{ma25['ma']:.2f}", ma25['trend']) # [AI MOD]
    t.add_row("MA60 (季線)", f"{ma60['ma']:.2f}", ma60['trend'])
    t.add_row("MA200 (年線)", f"{ma200['ma']:.2f}", ma200['trend'])

    bias_color = "bright_red" if abs(bias) > 20 else "white"
    bias_label = "⚠過熱" if abs(bias) > 20 else "正常"
    t.add_row("季線乖離", f"[{bias_color}]{bias:+.2f}%[/]",
    ...
    console.print(f"        MA200 扣抵 {ma200['deduction']:.2f} {'<' if ma200['deduction'] < close else '>'} 收盤 {close:.2f}，{d200_msg}")

def scan_market_stocks(conn: sqlite3.Connection, min_volume: int = 500, strat_choice: str = None, sort_choice: str = None) -> None:
    ...
    if _SCAN_CACHE['date'] == latest_date and _SCAN_CACHE['min_volume'] == min_volume and _SCAN_CACHE['results'] is not None:
            ...
            for stock in hit_list:
                ...
                try:
                    ...
                    if strat_choice == "1":  # 突破年線
                        if len(c) >= 200 and prev_price <= ma_series[-2] and curr_price > ma_series[-1] and curr_vol > prev_vol:
        ...
        assert fetch_count == len(hit_list), f"fetch_count={fetch_count} != len(hit_list)={len(hit_list)}"
...
def _analyze_one(conn, code, name_map) -> list:
    ...
    if prev_price <= ma60[-2] and curr_price > ma60[-1] and curr_vol > prev_vol:
        ...
        triggered.append({
            ...
            'prev_close': prev_price,  # [AI MOD] Include prev_close for coloring
            'ma60': ma60[-1],
            'ma200': ma200[-1] if ma200 is not None else 0.0, # [AI MOD] Guard against None ma200
            'target_ma': ma60[-1],
            'vol_ratio': vol_ratio,
            'bias': bias,
            'trend': "突破季線",
...
def _display_scan_results(results: list, latest_date: int, sort_choice: str, strat_choice: str) -> None:
    if not results:
