...
class SupportResistanceEngine:
    ...
    def _compute_atr(self, period: int = StrategyConfig.ATR_PERIOD):
        ...
        self.df['_tr'] = np.maximum(
            ...
            np.maximum(
                (self.df['high'] - self.df['_pc']).abs(),
                (self.df['low'] - self.df['_pc']).abs()
            )
        )
    ...
    def _find_swing(self, arr, dates, mode):
        ...
        for i in range(len(arr) - StrategyConfig.SWING_RIGHT - 1, StrategyConfig.SWING_LEFT - 1, -1):
            if i <= 0:
                break
        ...
        if len(arr) >= 10:
            ...
            return {"date": _to_date_int(dates[idx]), "price": float(seg[idx if mode == 'high' else np.argmin(seg)])}
        return None

    def _find_swing_points(self):
        recent = self.df.tail(StrategyConfig.SWING_WINDOW)
        return (self._find_swing(recent['high'].to_numpy(), recent['date'].to_numpy(), 'high'),
    ...
    def _recent_extremes(self):
        ...
        return (
            {"date": _to_date_int(tail.loc[max_idx, 'date']), "price": float(tail.loc[max_idx, 'high'])},
            {"date": _to_date_int(tail.loc[min_idx, 'date']), "price": float(tail.loc[min_idx, 'low'])},
        )
    ...
    def _key_close_levels(self):
        ...
        mask = (
            ...
            & (recent['high'] - recent['close'] <= StrategyConfig.KEY_CLOSE_NEAR_HIGH_RATIO * (recent['high'] - recent['low']))
    ...
    def _acceleration_band(self):
        ...
        vol_ma = recent['volume'].rolling(window=20).mean().to_numpy() if has_vol else np.zeros(len(closes))
        bw = StrategyConfig.ACCEL_BOX_WINDOW
        accel_idx = -1
        for i in range(len(closes) - 1, bw - 1, -1):
            price_ok = closes[i] > np.max(highs[i - bw:i])
            vol_ok = vols[i] >= StrategyConfig.ACCEL_VOL_MULTIPLIER * vol_ma[i] if vol_ma[i] > 0 else True
    ...
    def _merge_levels(self, levels, atr_ref):
        ...
        if not valid:
            return []
        tick = StrategyConfig.get_tick_size(self.last_close)
        tol = max(tick * 2, min(0.01 * self.last_close, StrategyConfig.MERGE_ATR_TOLERANCE * atr_ref))
        groups, cur = [], [valid[0]]
        for lv in valid[1:]:
    ...
    def _classify(self, swing_hi, swing_lo, key_closes, accel, density, recent_hi, recent_lo):
        ...
        if len(self.df) >= 2:
            ...
            raw.extend([P, 2*P - prev['low'], 2*P - prev['high'], P + (prev['high'] - prev['low']), P - (prev['high'] - prev['low'])])
        if len(self.df) > 0:
            cur = self.df.iloc[-1]
            ...
            raw.extend([2.0 * p_val - l_val, p_val + (h_val - l_val), h_val + 2.0 * (p_val - l_val)])
            raw.extend([2.0 * p_val - h_val, p_val - (h_val - l_val), l_val - 2.0 * (h_val - p_val)])
            raw.extend([c_val + atr_val, c_val + 2.0 * atr_val, c_val - atr_val, c_val - 2.0 * atr_val])
    ...
    def analyze(self):
        ...
        try:
            ...
            res_cand, sup_cand = self._classify(swing_hi, swing_lo, key_closes, accel, density, recent_hi, recent_lo)
            atr_col = self.df['atr']
            atr_ref = float(atr_col.iloc[-1]) if atr_col.iloc[-1] is not None and not np.isnan(atr_col.iloc[-1]) else self.last_close * 0.02
            ...
            ma25 = float(self.df['close'].rolling(window=25).mean().iloc[-1]) if len(self.df) >= 25 else self.last_close
            std25 = float(self.df['close'].rolling(window=25).std().iloc[-1]) if len(self.df) >= 25 else 0
            return {
...
def _render_mobile_sr(data, code, name):
    ...
    recent_res, recent_sup = data.get('recent_resistance_swing_high'), data.get('recent_support_swing_low')
    if recent_res or recent_sup:
        console.print(f"前高 {(recent_res['price'] if recent_res else 0):.2f} / 前低 {(recent_sup['price'] if recent_sup else 0):.2f}")
...
def _show_indicators(data):
    lc = data['last_close']
    all_levels = [x['level'] for x in data.get('merged_resistance_levels', []) + data.get('merged_support_levels', [])]
...
def _show_extras(data):
    ...
    if accel:
        console.print(f"vsbc(加速起漲): {accel['band_low']:.2f}~{accel['band_high']:.2f}(中心:{accel['vwap_center']:.2f})")
    ...
    while True:
        ...
        if ch == '0':
            ...
            if filtered:
                ...
                console.print(f"[green]✅ 已套用 {labels[key]} 篩選：{len(filtered)} / {len(all_scored)} 檔 (低點 {low_val:.2f}，價格 ≤ {low_val * 1.1:.2f})[/green]")
            else:
...
def _analyze_one(conn, code, name_map):
    ...
    if not result:
        return None
    return {'code': code, 'name': name_map.get(code) or FALLBACK_NAMES.get(code, '---'), 'analysis': result, 'df': df}
...
def _display_results(results, latest_date=0, sort_choice="1", min_volume=StrategyConfig.DEFAULT_MIN_VOLUME, current_filters=None):
    if not results:
