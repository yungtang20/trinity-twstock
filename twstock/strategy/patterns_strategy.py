...
class PivotBasedScanner:
        ...
        for i in range(len(lo_pivots)):
            for j in range(len(hi_pivots)):
                ...
                for foot2 in foot2s:
                    ...
                    if foot2["price"] < foot1["price"] + rebound * 0.5:
                        continue
                    neckline = peak["price"]
                    dist = abs(current - neckline) / neckline
                    if dist > NECKLINE_TOL:
                    ...
                    results.append(
                        self._make_pattern(
                            ...
                            foot1["price"],
                            [
                                (foot1["date"], foot1["price"]),
                                (peak["date"], neckline),
                                (foot2["date"], foot2["price"]),
                            ],
                            (foot1["idx"], foot2["idx"]),
                        )
                    )
    ...
    def _match_hs_bottom(self, df, lo_pivots, hi_pivots) -> List[PatternInfo]:
        ...
        for i in range(len(lo_pivots) - 2):
            ...
            if ls["price"] < mid or rs["price"] < mid:
                continue
    ...
    def _match_triple_bottom(self, df, lo_pivots, hi_pivots) -> List[PatternInfo]:
        ...
        for i in range(len(lo_pivots) - 2):
            ...
            avg = np.mean(prices)
            if any(abs(p - avg) / avg > 0.08 for p in prices):
                continue
    ...
    def _match_v_reversal(self, df, lo_pivots) -> List[PatternInfo]:
        ...
        for p in lo_pivots:
            ...
            rebound_pct = (
                (post_high - p["price"]) / (pre_high - p["price"]) if pre_high > p["price"] else 0
            )
            if rebound_pct < 0.50:
                continue
            neckline = pre_high
            ...
            results.append(
                self._make_pattern(
                    ...
                    p["price"],
                    [
                        (df.index[max(0, idx - 10)], pre_high),
                        (p["date"], p["price"]),
    ...
    def _match_inv_v(self, df, hi_pivots) -> List[PatternInfo]:
        ...
        for p in hi_pivots:
            ...
            drop_pct = (
                (p["price"] - post_low) / (p["price"] - pre_low) if p["price"] > pre_low else 0
            )
            if drop_pct < 0.50:
                continue
            neckline = pre_low
            ...
            if abs(current - neckline) / neckline <= NECKLINE_TOL:
                results.append(
                    PatternInfo(
                        ...
                        extreme=round(last_hi, 2),
                        target=round(last_hi, 2),
                        stop_loss=round(last_lo, 2),
                        confidence=0.8,
                        quality=0.0,
                        points=pts,
                        zone=zone,
            ...
            if abs(current - neckline) / neckline <= NECKLINE_TOL:
                results.append(
                    PatternInfo(
                        ...
                        direction="bearish",
                        neckline=round(neckline, 2),
                        extreme=round(last_lo, 2),
                        target=round(last_lo, 2),
                        stop_loss=round(last_hi, 2),
                        confidence=0.8,
                        quality=0.0,
                        points=pts,
                        zone=zone,
                        ...
                        if dist <= NECKLINE_TOL:
                            results.append(
                                self._make_pattern(
                                    ...
                                    float(min(fg)),
                                    [
                                        (df.index[0], float(fp[0])),
                                        (df.index[peak_idx], neckline),
                                        (df.index[-1], float(fg[-1])),
                                    ],
                                    (0, n - 1),
                                )
                            )
    ...
    def _match_range_box(self, df, hi_pivots, lo_pivots) -> List[PatternInfo]:
        ...
        if hi_spread > 0.06 or lo_spread > 0.06:
            return results
    ...
    def _match_arc(self, df, pivots, direction) -> List[PatternInfo]:
        ...
        if direction == "bullish":
            ...
            if not (0.2 * n <= min_idx <= 0.8 * n):
                return results
            start_p, end_p = prices[0], prices[-1]
            if start_p <= min_val or end_p <= min_val:
                return results
            ...
            if not (0.2 * n <= max_idx <= 0.8 * n):
                return results
            start_p, end_p = prices[0], prices[-1]
            if start_p >= max_val or end_p >= max_val:
                return results
    ...
    def _make_pattern(name, direction, neckline, extreme, points, zone):
        ...
        if direction == "bullish":
            ...
            target = (
                round(neckline + height, 2) if neckline > extreme else round(neckline - height, 2)
            )
            stop_loss = extreme
        return PatternInfo(
            pattern=name,
...
class StockPredictionAnalyzer:
    ...
    def _render_mobile_patterns(self, df, code, name, pat):
        ...
        if pat:
            ...
            rconsole.print(
                f"  頸線 {pat.neckline:.2f}  目標 {pat.target:.2f}  停損 {pat.stop_loss:.2f}"
            )
        else:
...
class MarketScanner:
    ...
    def _display(self, preds, ld):
        ...
        t = Table(
            title=f"📈 預測分析榜 (基準日: {ld})", box=box.SIMPLE, border_style="cyan", expand=False
        )
        for c, s in [
            ("代號", "magenta"),
            ("名稱", None),
...
class PatternBreakoutScanner:
    ...
    def scan(self, min_volume=500, direction_filter=None):
        try:
            ...
            if direction_filter:
                ...
                cands_with_data = [
                    (c, d) for c, d in cands_with_data if c.direction == direction_filter
                ]

            if not cands:
                label = {"bullish": "看漲", "bearish": "看跌", "neutral": "區間整理"}.get(
                    direction_filter, ""
                )
    ...
    def _scan_one(self, symbol, name):
        ...
        cand = BreakoutCandidate(
            ...
            predicted_break_day=0,
            predicted_break_price=0.0,
            predicted_peak=0.0,
            volume=volume,
            points=best.points,
    ...
    def _display(self, cands, ld, sort_choice="1"):
        ...
        def _tbl(title, rows, border):
            ...
            t = Table(
                title=title, box=box.ROUNDED, border_style=border, title_style=f"bold {border}"
            )
            ...
            for r in rows:
                ...
                t.add_row(
                    ...
                    f"[{ds}]{r.pattern}[/]",
                    f"{r.neckline:.2f}",
                    f"[bright_cyan]{r.target:.2f}[/]",
                    f"[bright_red]{r.stop_loss:.2f}[/]",
                )
            rconsole.print(t)

        if bulls:
            _tbl(f"🔴 看漲型態 · {base}", bulls, "bright_red")
            rconsole.print()
        if bears:
    ...
    def pattern_scan_market(self, conn, mv=None, pattern_filter=None):
        ...
        try:
            ...
            while True:  # category loop
                ...
                rconsole.print(
                    "  [bold][1][/] 看漲型態（W底·N字底·頸肩底·三重底·V反轉·圓弧底·上升三角·下降楔形·上升通道·牛旗）"
                )
                rconsole.print("  [bold][2][/] 區間整理（箱型·對稱三角）")
                rconsole.print(
                    "  [bold][3][/] 看跌型態（M頭·頸肩頂·三重頂·倒V·圓弧頂·下降三角·上升楔形·下降通道·熊旗）"
                )
