        ...
        if choice == "1":  # 撐壓分析
            while True:
                ...
                if ans in filter_map:
                    vol = _input_vol()
                    run_strategy_by_id(choice, {'scan': True, 'vol': vol, 'filter': filter_map[ans]})
                    _prompt_kronos_ai()
                    continue
            ...
            while True:
                ...
                if ans in ('1', '2', '3'):
                    ...
                    if not sort_choice:
                        break
                    run_strategy_by_id(choice, {'scan': True, 'vol': vol, 'strat_choice': ans, 'sort_choice': sort_choice})
                    _prompt_kronos_ai()
                    continue
            ...
            while True:
                ...
                if ans in ('1', '2'):
                    ...
                    run_strategy_by_id(choice, {'scan': True, 'strat_choice': ans, 'n_days': n_days, 'sort_choice': sort_choice})
                    _prompt_kronos_ai()
                    continue
                ...
                if ans == '3':
                    ...
                    run_strategy_by_id(choice, {'scan': True, 'strat_choice': '3', 'sort_choice': sort_choice})
                    _prompt_kronos_ai()
                    continue
            ...
            while True:
                ...
                console.print("  [1] 看漲型態（W底·N字底·頸肩底·三重底·V反轉·圓弧底·上升三角·下降楔形·上升通道·牛旗）")
                console.print("  [2] 區間整理（箱型·對稱三角）")
                console.print("  [3] 看跌型態（M頭·頸肩頂·三重頂·倒V·圓弧頂·下降三角·上升楔形·下降通道·熊旗）")
                console.print("  [4] 全部")
                console.print("  [Enter] 回到上一頁")
                ans = get_single_key_input(
                    "🔍 輸入股號或按 Enter 回到上一頁: ",
                ...
                if ans in filter_map:
                    vol = _input_vol()
                    run_strategy_by_id(choice, {'scan': True, 'vol': vol, 'pattern_filter': filter_map[ans]})
                    _prompt_kronos_ai()
                    continue
