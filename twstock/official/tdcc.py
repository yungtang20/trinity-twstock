...
def fetch_single_stock_tdcc_from_portal(
    stock_id: str, date_str: str, session: Optional[requests.Session] = None
) -> Optional[dict]:
    """
...
def fetch_tdcc_historical(weeks: int = 1, retries: int = 2) -> pd.DataFrame:
    ...
    for week_offset in range(weeks):
        ...
        if week_offset > 0:
            ...
            print(
                f"  → 略過 TDCC 歷史日期: {date_str} (因 OpenAPI 不支援歷史下載，將於選股時按需補爬)",
                flush=True,
            )
            continue
                ...
                for code, levels in stock_levels.items():
                    ...
                    for lvl, lv_data in levels.items():
                        if lvl == 17:  # TDCC OpenAPI 合計是 level 17
                            total_shares = int(lv_data["shares"])
                            total_people = int(lv_data["people"])
                        elif lvl == 15:  # 大股東 (持股 > 1000張) 持股比例
                            whale_shares += int(lv_data["shares"])
                            whale_people += int(lv_data["people"])  # [AI MOD]
                    if total_shares == 0:
                        continue
