...
class TestTC6AdjFactor:
    """TC6: adj_factor 預設值必須是 1.0"""

    def test_adj_factor_defaults_to_one(self, fetcher, raw_2330_3days):
        rows = fetcher._transform(raw_2330_3days)
        for r in rows:
            assert r["adj_factor"] == 1.0, (
                f"adj_factor 應為 1.0，實際 {r['adj_factor']}"
            )
...
class TestTC10Integration:
    ...
    def test_fetch_and_save_column_values(self, fetcher, db):
        ...
        cur = db.execute(
            ...
            "adj_factor, source FROM stock_history "
            "WHERE stock_id='2330' AND date='2024-01-02'"
        )
        row = cur.fetchone()
        ...
        assert row[8] == "finmind", f"source 錯：{row[8]}"
