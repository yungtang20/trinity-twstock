...
def test_stock_history_core_columns(db_conn):
    ...
    assert expected.issubset(columns.keys()), (
        f"缺少欄位: {expected - set(columns.keys())}"
    )
...
def test_stock_history_volume_amount_are_integer(db_conn):
    ...
    assert columns.get("volume") == "INTEGER", (
        f"volume 應為 INTEGER，實際 {columns.get('volume')}"
    )
    assert columns.get("amount") == "INTEGER", (
        f"amount 應為 INTEGER，實際 {columns.get('amount')}"
    )
...
def test_tdcc_shareholding_is_view(db_conn):
    ...
    assert objects["tdcc_shareholding"] == "view", (
