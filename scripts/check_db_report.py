...
print("=== 資料庫完整性檢查報告 ===")
print(
    f'檢查時間: {datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).strftime("%Y-%m-%d %H:%M:%S")} (台北時間)'
)
print(f"資料庫路徑: {db}")
...
try:
    r = subprocess.run(["sqlite3", db, ".tables"], capture_output=True, text=True, timeout=10)
    if r.returncode == 0:
